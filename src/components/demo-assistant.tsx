...
} from '@/lib/domain/demo-assistant';
...
export function DemoAssistant() {
  ...
  function send(text: string) {
    ...
    setMessages((history) => [
      ...
      { role: 'assistant', text: answer.text, link: answer.link },
    ]);
  ...
  return (
    <>
      <button
        ...
        onClick={launch}
        aria-label="Buka AIDA, pembantu digital Dasar Dadah Negara"
        aria-expanded={open}
        aria-controls="ddn-assistant"
      >
        <Image src="/aida-avatar.svg" alt="" width={48} height={48} />
        <span>
          <strong>AIDA</strong>
          <small>
            Mahu panduan? <MessageCircle size={12} />
          </small>
        </span>
      </button>
      {open && (
        <>
          ...
          <section
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === 'Tab') {
                const nodes =
                  dialog.current?.querySelectorAll<HTMLElement>('button, a[href], input');
          ...
          >
            <header className="assistant-header">
              ...
              <div>
                <h2 id="assistant-title">AIDA</h2>
                <p>Asisten Inteligen Dasar Antidadah</p>
              </div>
              <button className="icon-button" onClick={reset} aria-label="Mulakan perbualan semula">
                <RotateCcw size={17} />
              </button>
              <button
                ref={closeButton}
            ...
            <div
              ...
              role="log"
              aria-label="Perbualan AIDA"
              aria-live="polite"
              aria-relevant="additions text"
            >
              {messages.map((message, i) => (
                <div key={i} className={`assistant-message ${message.role}`}>
                  <small>{message.role === 'assistant' ? 'AIDA' : 'ANDA'}</small>
                  <p>{message.text}</p>
                  {message.link && (
                    <Link href={message.link.href} onClick={close}>
                      {message.link.label}
            ...
            <div className="assistant-choices" aria-label="Cadangan soalan">
              {choices.map((choice) => (
                <button key={choice} onClick={() => send(choice)}>
                  {choice}
            ...
            >
              ...
              <input
                ...
                autoComplete="off"
              />
              <button type="submit" aria-label="Hantar soalan" aria-disabled={!input.trim()}>
                <Send size={18} />
              </button>
            </form>
            <p className="assistant-privacy">
              Mesej hanya dalam sesi ini. Elakkan maklumat peribadi.
            </p>
          </section>
        </>
      )}
