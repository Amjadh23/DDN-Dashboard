...
export async function TerasView({
  teras,
  p,
  embedded = false,
}: {
  ...
  const info = terasInfo[teras],
    session = await getSession();
  // Embedded on the overview, this Teras keeps its own links pointing back at
  // its section rather than at a page of its own.
  const base = embedded ? '/' : `/teras/${teras}`,
    hash = embedded ? '#pendidikan-pencegahan' : '';
                ...
                <nav className="button-row" aria-label="Tetapan sumber rawatan">
                  {[
                    ...
                    <Link
                      ...
                      href={`${base}?${filterQuery(filters)}&perspective=${perspective}&setting=${setting}${hash}`}
                    >
                      {label}
                    </Link>
                  ))}
  ...
  const className = `teras-page accent-${info.accent}`;
  return embedded ? (
    <section id="pendidikan-pencegahan" className={`overview-teras-section ${className}`}>
      {content}
    </section>
  ) : (
